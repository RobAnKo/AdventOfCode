classdef Conway3D
    ...
    methods
        function obj = Conway3D(matrix)
            bin_matrix = matrix == '#';
            obj.Matrix = bin_matrix;
            obj.MatrixSize = [size(matrix, 1), size(matrix, 2), size(matrix, 3)];
        ...
        function cm = create_copy_matrix(obj)
            ...
            cm(2:end-1, 2:end-1, 2:end-1) = obj.Matrix;
        end

        
        function status = find_status(obj,suby,subx,subz)
            if obj.CopyMatrix(suby,subx,subz)
                if any(obj.neighbour_count(suby,subx,subz) == [2,3])
                    status = 1;
                else
                    status = 0;
                end
            else
                if obj.neighbour_count(suby,subx,subz) == 3
                    status = 1;
                else
                    status = 0;
                end
        ...
        function n = neighbour_count(obj, suby, subx, subz)
            ...
            n = sum(obj.CopyMatrix(ys,xs, zs),'all')-obj.CopyMatrix(suby,subx,subz);
        end
        ...
        function obj = update(obj,n)
            ...
            for i = 1:n
                ...
                [ys,xs,zs] = arrayfun(@(idx) ind2sub(obj.NextStepMatrixSize, idx), index_range);
                new_vals = arrayfun(@(idx) obj.find_status(ys(idx),xs(idx),zs(idx)), index_range);
